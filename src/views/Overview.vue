    ...
    >
      <template v-slot:default="{ items }">
        <v-card
          ...
          color="#EEEEEE"
        >
          <v-banner color="#222831" class="white--text" style="font-size: 22px"
            >{{ map.map[0] }} 线 {{ map.map.substring(2) }} 洞</v-banner
          >
          <div class="d-flex flex-wrap">
            <v-card
              ...
              :key="shop.index"
            >
              <v-banner color="#00ADB5" class="white--text">{{
                shop.title
              }}</v-banner>
              <v-divider></v-divider>
              <v-list dense>
                <v-list-item v-for="good in shop.goods" :key="good.index">
                  <v-tooltip right transition="none">
                    ...
                    <v-img
                      :src="`${starttime}/${map.map}/${good.index}_detail.jpg`"
                    ></v-img>
                  </v-tooltip>
                </v-list-item>
              </v-list>
